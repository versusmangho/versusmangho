...
(function (global) {
    ...
    function nameMayBeSame(p, e) {
        ...
        if (!rp || unmatched(r.m, dilated(n.m)) > ratioOf(r, n) * rp) return false;
        // 배경 그림이 크게 섞인(noisy) 평소 모양은 남는 픽셀이 사람마다 달라 기준이 없다 — 포함만 본다.
        // 남는 픽셀은 닉네임 폭(READY 폭) 안에서만 센다 — 그 뒤의 명패 그림은 닉네임과 상관없다
        return n.noisy || unmatched(cutTo(n.m, r.m.length + NAME_SPAN_TAIL), dilated(r.m)) <= COVER_EXCESS_MAX * rp;
    }
    // 장부에 p의 상태 모양이 없어 다른 상태 모양과 비교하게 되는 항목인가
    const crossState = (p, e) => !!p.nameState && !(e.names && e.names[p.nameState]);
    ...
    /* 후보 목록에서 같은 사람 찾기.
       ...
       opts.loose: 장부에 지금 상태(READY/평소)의 닉네임 모양이 없어 안 맞은 후보도, READY 글자가 평소 모양 안에
         들어가면(nameMayBeSame) 받는다 — 평소 모양에 섞인 배경 부스러기만큼 어긋난 것일 수 있다.
    ...
    function findMatch(fp, candidates, getFp, opts = {}) {
        ...
        if (fp.nameState) {
            ...
            // (opts.loose면 상태가 달라 안 맞은 항목도 — READY 글자가 평소 모양 안에 들어갈 때만)
            pool = all.filter(n => {
                const c = n.nc !== undefined ? n.nc : nameCompare(fp, n.fp);
                return !c || (opts.loose && crossState(fp, n.fp) && nameMayBeSame(fp, n.fp));
            });
        }
    ...
    global.VMH.Room = {
        ...
        nameMaskOf, nameDist, nameCompare, nameMayBeSame, mergeName,
