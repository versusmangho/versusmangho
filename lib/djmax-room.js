...
(function (global) {
    ...
    const BLOB_MAX = 2;           // 5x5가 꽉 찬 픽셀이 이보다 많으면 배경 그림이 크게 섞인 것
    ...
    function nameMaskOf(plateCanvas) {
        ...
        const badgeAt = (x) => {
            ...
            for (let j = x; j < Math.min(rw, x + BADGE_W); j++) {
                if (colorIn(j, magenta) >= 4) pink++;
                star += colorIn(j, yellow);
            }
        ...
        // 글자 획은 2~3px 두께다. 5x5가 통째로 찬 곳이 있으면 밝은 배경 그림이 크게 섞인 것 — 닉네임으로 가르지 않는다
        let blobs = 0;
    ...
    function makeFingerprint(plateCanvas) {
        ...
        const names = {};
        if (nm) names[nameState] = { m: nm.m, h: plateCanvas.height };
        return {
    ...
    function fpNameSame(a, b) { const c = nameCompare(a, b); return !c || c.d <= c.limit; }
    ...
    function findMatch(fp, candidates, getFp) {
        ...
        if (fp.nameState) {
            ...
            const named = all.filter(n => fpAvatarSame(fp, n.fp))
                .map(n => Object.assign(n, { nc: nameCompare(fp, n.fp) }))
                .filter(n => n.nc && n.nc.d <= n.nc.limit)
                .sort((x, y) => x.nc.d - y.nc.d);
            if (named.length) {
                if (named.length > 1 && named[1].nc.d - named[0].nc.d < NAME_AMBIGUOUS_GAP) return { ambiguous: true };
            ...
            pool = all.filter(n => !nameCompare(fp, n.fp));
        }
        ...
        return { match: best.cand, dist: best.d, refreshName: needsNameRefresh(fp, best.fp) };
    }
