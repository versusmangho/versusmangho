...
(function (global) {
    ...
    // 같은 검사에서 READY 글자 중 평소 모양에 짝이 없어도 되는 몫. 칸이 바뀌면 글자가 1~2px 밀려 엄격한 기준(0.03)을 넘는다
    // (tla8405: 7번 칸 READY ↔ 8번 칸 평소 0.030). 실측(images/, 아바타가 같은 READY×평소 341쌍): 동일인 최대 0.076 / 타인 최소 0.203
    const COVER_MISS_MAX = 0.1;
    ...
    function sameReading(p, q) {
        if (!fpAvatarSame(p, q) || p.nameState !== q.nameState) return false;
        const st = p.nameState;
