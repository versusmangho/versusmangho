...
(function (global) {
    ...
    /* ── 닉네임 글자 지문 ─────────────────
       ...
       평소엔 흰 글자(min(R,G,B)가 높다 — 파란 띠·색 뱃지는 빠진다), READY 때는 주황 배경 위 검은 글자라
       명패마다 배경이 주황인지 먼저 보고 밝은 쪽/어두운 쪽 중 하나를 글자로 잡는다. 글꼴은 같으니 모양은 그대로 비교된다.
       아래 줄의 레벨은 판마다 오를 수 있으므로 쓰지 않는다 — 윗줄 덩어리만 잡는다.
       명패를 높이 94px로 맞춘 뒤 읽으므로 로비·결과 화면·해상도에 따른 크기 차이를 흡수한다.
       배경 그림이 밝은 명패(흰·분홍 그림 위 흰 글자)는 글자만 떼어낼 수 없어 null — 그 사람은 아바타로만 가린다. */
    const NAME_NORM_H = 94;       // 명패를 이 높이로 늘리거나 줄여서 읽는다 → 아래 픽셀 값은 모두 이 크기 기준
    const NAME_X0 = 94;           // 아바타 칸(~88px)과 파란 띠를 건너뛴 곳부터 찾는다
    const NAME_X1 = 0.72;         // 명패 너비 비율 — 오른쪽 캐릭터 그림 앞까지만
    const NAME_SCAN_Y1 = 40;      // 윗줄을 찾는 세로 범위. 레벨 숫자는 그 아래(37px~)에서 따로 떨어진 덩어리다
    const NAME_MIN_H = 8;         // 이보다 낮은 가로줄 덩어리는 배경 티끌로 본다
    const NAME_ROWS = 24;         // 닉네임 줄 높이 — 세로줄 하나를 정수 하나(비트 24개)에 담는다. 더 내려가면 'LV.'가 걸린다
    const BORDER_ROW_SHARE = 0.8; // 이보다 넓게 찬 가로줄은 명패 테두리
    const BADGE_W = 12;           // 닉네임 뒤 '+' 뱃지 폭(≈18px) 중 이만큼을 훑어 절반 넘게 분홍이면 뱃지
    const BLOB_MAX = 2;           // 5x5가 꽉 찬 픽셀이 이보다 많으면 배경 그림이 섞인 것
    const NAME_GAP = 24;          // 글자 없는 폭이 이만큼 이어지면 닉네임이 끝난 것 (뒤의 배경 티끌을 자른다)
    const NAME_CHUNK = 16;        // 따로 맞추는 조각 폭 (글자 하나 ≈ 20px)
    const NAME_START_SHIFT = 3;   // 첫 조각을 좌우로 미는 범위 — 둘 다 첫 글자 열에서 시작하므로 작다
    const NAME_DRIFT = 2;         // 다음 조각이 앞 조각보다 더 밀릴 수 있는 폭
    const TEXT_MIN = 130;         // min(R,G,B)가 이 이상이면 흰 글자 픽셀 (축소돼 번진 가는 획까지 남긴다)
    const DARK_TEXT_MAX = 155;    // READY 때: R이 이 이하면 검은 글자 픽셀. 주황(R 255)과 검정(R 18)이 가장 크게 갈리는 채널이
                                  // R이라 축소돼 주황과 섞인 가는 획도 남는다. 획이 차지하는 비율로 흰 글자 기준(130)과 맞춘 값(≈42%)
    const READY_R_MIN = 150, READY_RB_MIN = 90;   // 주황 픽셀 — 빨강이 높고 파랑이 낮다
    const READY_ORANGE_SHARE = 0.4;               // 닉네임 칸의 이만큼이 주황이면 READY로 본다
    // 같은 사람 기준 = 짝 없는 픽셀 수 ÷ 글자 픽셀 수(둘 중 적은 쪽). 긴 닉네임일수록 흔들림도 커지므로 비율로 본다.
    // 실측(images/ 로비 4장·결과 2장, 글자를 읽은 명패 19장): 동일인 15쌍 모두 0 — READY↔평소, 로비↔결과, 레벨 635↔637 포함
    // / 타인 최소 0.239. 한 획 차이 닉네임은 실제 스샷에 없어 합성(맑은 고딕)으로 쟀다: 동일인 최대 0.022 / 도기↔모기 0.042
    ...
    /* 닉네임 줄을 이진화한 모양 → 세로줄마다 정수 하나(비트 r = 윗줄 꼭대기에서 r번째 픽셀).
       글자를 못 찾으면 null */
    function nameMaskOf(plateCanvas) {
        ...
        const rw = Math.round(sw * k), rh = NAME_SCAN_Y1;
        ...
        let orange = 0;
        for (let i = 0; i < d.length; i += 4) if (d[i] >= READY_R_MIN && d[i] - d[i + 2] >= READY_RB_MIN) orange++;
        if (orange >= (d.length / 4) * READY_ORANGE_SHARE) {
            for (let i = 0, p = 0; i < d.length; i += 4, p++) on[p] = d[i] <= DARK_TEXT_MAX ? 1 : 0;
        } else {
            for (let i = 0, p = 0; i < d.length; i += 4, p++) on[p] = Math.min(d[i], d[i + 1], d[i + 2]) >= TEXT_MIN ? 1 : 0;
        }

        // 가로로 거의 꽉 찬 줄은 명패 테두리다 (READY 땐 위 테두리도 어둡게 잡힌다) — 글자가 아니므로 지운다
        for (let y = 0; y < rh; y++) {
            let n = 0;
        ...
        // 윗줄 덩어리의 꼭대기 — 글자 픽셀이 있는 가로줄이 NAME_MIN_H 이상 이어지는 첫 구간
        let top = -1, runStart = -1;
        for (let y = 0; y <= rh; y++) {
            let n = 0;
            if (y < rh) for (let x = 0; x < rw; x++) n += on[y * rw + x];
            if (n >= 2) { if (runStart < 0) runStart = y; continue; }
            if (runStart >= 0 && y - runStart >= NAME_MIN_H) { top = runStart; break; }
            runStart = -1;
        }
        if (top < 0) return null;
        const bottom = Math.min(rh, top + NAME_ROWS);
        const column = (x) => {
            let bits = 0;
            for (let y = top; y < bottom; y++) if (on[y * rw + x]) bits |= 1 << (y - top);
            return bits;
        };
        // 닉네임 뒤의 분홍 '+' 뱃지 — 여기서 닉네임이 끝난다
        const magentaCol = (x) => {
            let n = 0;
            for (let y = top; y < bottom; y++) {
                const i = (y * rw + x) * 4;
                if (d[i] >= 170 && d[i + 2] >= 150 && d[i + 1] <= 100) n++;
            }
            return n >= 4;
        };
        const badgeAt = (x) => {
            let n = 0;
            for (let j = x; j < Math.min(rw, x + BADGE_W); j++) if (magentaCol(j)) n++;
            return n >= BADGE_W / 2;
        };

        // 닉네임의 좌우 끝 — 첫 글자 열부터, 뱃지나 NAME_GAP만큼의 빈 폭이 나오기 전까지
        const mask = [];
        let last = -1;
        for (let x = 0; x < rw; x++) {
            if (last >= 0 && magentaCol(x) && badgeAt(x)) break;
            const bits = column(x);
            if (!bits) { if (last >= 0 && x - last > NAME_GAP) break; continue; }
            if (last >= 0) for (let g = last + 1; g < x; g++) mask.push(0);
            mask.push(bits);
            last = x;
        }
        if (!mask.length) return null;

        // 글자 획은 2~3px 두께다. 5x5가 통째로 찬 곳이 있으면 밝은 배경 그림이 섞인 것 —
        // 이런 모양은 READY(배경이 주황으로 가려짐)와 평소가 서로 달라 같은 사람을 남으로 만든다. 닉네임으로 가르지 않는다
        let blobs = 0;
        for (let i = 2; i < mask.length - 2; i++) {
            const v = mask[i - 2] & mask[i - 1] & mask[i] & mask[i + 1] & mask[i + 2];
            const solid = v & (v >> 1) & (v >> 2) & (v >> 3) & (v >> 4);
            blobs += IH.popcnt32(solid);
        }
        if (blobs > BLOB_MAX) return null;
    ...
    /* 두 닉네임 모양의 거리 = 서로 짝이 없는 글자 픽셀 수의 합 (0이면 같은 모양).
       합성 실측(맑은 고딕 12개 닉네임 × READY 덮개 위·아래 · 1080p/900p 축소 · 1~2px 어긋남):
       기준은 글자 픽셀 수에 대한 비율로 잡는다 — NAME_SAME_RATIO 참고 */
    function nameDist(a, b) {
        if (!a || !b || !a.length || !b.length) return Infinity;
        const da = a._dil || (a._dil = dilate(a)), db = b._dil || (b._dil = dilate(b));
        return unmatched(a, db) + unmatched(b, da);
    }

    /* 명패 한 칸의 지문. 아바타 칸(grid·해시)으로 먼저 추리고, 닉네임 모양(name)으로 가른다 */
    function makeFingerprint(plateCanvas) {
        ...
        return {
            ...
            name: nameMaskOf(plateCanvas),
            nameH: plateCanvas.height
        };
    }

    function sameScale(a, b) {
        return !!a.nameH && !!b.nameH && Math.abs(a.nameH - b.nameH) <= NAME_SCALE_TOL * Math.max(a.nameH, b.nameH);
    }
    ...
    /* 같은 사람으로 볼 닉네임 거리의 상한. 크기가 다르면 느슨한 기준 */
    function nameLimit(a, b) {
        const ratio = sameScale(a, b) ? NAME_SAME_RATIO : NAME_SAME_RATIO_XRES;
        return ratio * Math.min(textPixels(a.name), textPixels(b.name));
    }
    ...
    function fpNameSame(a, b) { return !a.name || !b.name || nameDist(a.name, b.name) <= nameLimit(a, b); }
    ...
    /* 후보 목록에서 같은 사람 찾기.
       → { match, dist, refreshName } | null(없음) | { ambiguous: true }(엇비슷해 판단 보류)
       refreshName: 장부의 닉네임 지문이 없거나(옛 항목) 다른 크기에서 뜬 것 — 호출부가 지금 것으로 바꿔 둘 것 */
    function findMatch(fp, candidates, getFp) {
        const all = [];
        ...
        if (fp.name) {
            ...
            const named = near.filter(n => n.fp.name)
                .map(n => Object.assign(n, { nd: nameDist(fp.name, n.fp.name) }))
                .filter(n => n.nd <= nameLimit(fp, n.fp))
                .sort((x, y) => x.nd - y.nd);
            if (named.length) {
                if (named.length > 1 && named[1].nd - named[0].nd < NAME_AMBIGUOUS_GAP) return { ambiguous: true };
                return { match: named[0].cand, dist: named[0].d, refreshName: !sameScale(fp, named[0].fp) };
            }
            // 닉네임이 맞는 사람이 없다 → 닉네임 지문이 없는 옛 항목만 아바타로 이어 줄 수 있다
            pool = all.filter(n => !n.fp.name);
        }

        let best = null, secondD = Infinity;
        for (const n of pool) {
            if (!best || n.d < best.d) { if (best) secondD = best.d; best = n; }
            else if (n.d < secondD) secondD = n.d;
        }
        if (!best || !fpAvatarSame(fp, best.fp)) return null;
        if (secondD < Infinity && best.d * AMBIGUOUS_RATIO > secondD) return { ambiguous: true };
        return { match: best.cand, dist: best.d, refreshName: !!fp.name && !best.fp.name };
    }
    ...
    global.VMH.Room = {
        ...
        makeFingerprint, fpColorDist, fpHamDist, fpSame, findMatch, isDefaultPlate, plateThumb,
        nameMaskOf, nameDist,
