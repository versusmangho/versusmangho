...
(function (global) {
    ...
    async function dispatch(frame, W, H, info) {
        for (const sub of subscribers) {
            if (!wants(sub.name)) continue;
            try { await sub.handler(frame, W, H, info); }
    ...
    const Hub = {
        ...
        /* 틱을 기다리지 않고 지금 프레임 한 장 — 성과 스캔처럼 자기 박자로 읽는 쪽이 쓴다.
           받을 곳(target)과 무관하다: 사용자가 직접 시작한 일이라서 */
        grab: () => (session && session.isRunning()) ? session.grab() : Promise.resolve(null),
        setFrameRate: (fps) => session ? session.setFrameRate(fps) : Promise.resolve(),

        /* 공유가 멈춰 같은 그림만 오는 중인지 — 가려진 게임 창은 화면을 새로 그리지 않는다.
           그 프레임으로 명단을 고치거나 키를 누르면 안 된다 (lib/screen-capture.js 주석 참고) */
        isStale: () => !!(session && session.isRunning() && session.isStale()),
        frameAge: () => session && session.isRunning() ? session.frameAge() : 0,
