...
(function (global) {
    ...
    /* ── 화면이 멈췄는지 ─────────────────
       다른 창이 게임을 덮으면 게임은 화면을 새로 그리지 않는다. 그러면 두 가지가 같이 일어난다:
         · ImageCapture.grabFrame()은 '다음 새 프레임'을 기다리느라 영영 끝나지 않는다
           → busy가 풀리지 않아 틱이 통째로 멈춘다 (그래서 GRAB_TIMEOUT_MS로 끊고 비디오 요소로 넘어간다)
         · 비디오 요소는 마지막으로 받은 그림을 그대로 내놓는다 → 옛 화면을 지금 화면으로 착각한다
       옛 프레임으로 명단을 고치거나(입·퇴장) 키를 누르면(자동 방장 봇) 안 되므로, 프레임을 작게 줄여
       지문을 떠 두고 내용이 마지막으로 바뀐 시각을 센다. 게임 화면은 로비에서도 배경이 늘 움직이므로
       "한참 그대로 = 공유가 멈춤"으로 봐도 된다. 다시 그려지기 시작하면 저절로 풀린다. */
    const SIG_W = 64, SIG_H = 36;
    const STALE_MS = 1500;          // 내용이 이만큼 그대로면 멈춘 것으로 본다 (틱 3번)
    const GRAB_TIMEOUT_MS = 400;    // grabFrame이 이 안에 안 끝나면 기다리지 않는다

    let sigCanvas = null;
    function frameSig(src, W, H) {
        if (!sigCanvas) { sigCanvas = document.createElement('canvas'); sigCanvas.width = SIG_W; sigCanvas.height = SIG_H; }
        const ctx = sigCanvas.getContext('2d', { willReadFrequently: true });
        try { ctx.drawImage(src, 0, 0, W, H, 0, 0, SIG_W, SIG_H); } catch (e) { return null; }
        const d = ctx.getImageData(0, 0, SIG_W, SIG_H).data;
        let a = 0, b = 0;
        for (let i = 0; i < d.length; i += 4) { a = (a * 31 + d[i] + d[i + 1] * 3 + d[i + 2] * 7) | 0; b = (b + a) | 0; }
        return a + ':' + b;
    }
    ...
    /* 화면 공유 세션 하나.
       onFrame(frame, W, H, info)은 게임 화면만 잘라낸 프레임으로 intervalMs마다 불린다(앞 호출이 끝난 뒤에만).
       info = { stale, age } — stale이면 공유가 멈춰 같은 그림이 계속 오는 중이니 믿으면 안 된다.
    ...
    function createSession(opts) {
        ...
        const state = { stream: null, video: null, imageCapture: null, stopTicker: null, busy: false, rectKey: null, rect: null, cropped: null,
                        sig: null, changedAt: 0 };

        const isRunning = () => !!state.stream;
        const areaNote = () => state.cropped ? ' · ' + cropNote(state.cropped) : '';

        // 프레임 지문을 떠 내용이 마지막으로 바뀐 시각을 갱신한다
        function noteFrame(raw, W, H) {
            const sig = frameSig(raw, W, H);
            if (sig === null || sig === state.sig) return;
            state.sig = sig; state.changedAt = Date.now();
        }
        const frameAge = () => state.changedAt ? Date.now() - state.changedAt : 0;
        const isStale = () => frameAge() > STALE_MS;
        const freshness = () => ({ stale: isStale(), age: frameAge() });
        ...
        /* grabFrame은 '다음 새 프레임'을 기다린다 — 가려져서 다시 그려지지 않는 창에서는 영영 끝나지 않아
           busy가 풀리지 않고 인식이 통째로 멈춘다. 시간을 끊고 비디오 요소로 넘어간다
           (늦게 도착한 프레임은 아무도 안 쓰므로 여기서 닫아 준다) */
        async function grabFrame() {
            if (state.imageCapture) {
                let late = false;
                const p = state.imageCapture.grabFrame();
                p.then(b => { if (late && b && b.close) b.close(); }, () => { /* 아래 catch가 받는다 */ });
                const bmp = await Promise.race([p.catch(() => null), sleep(GRAB_TIMEOUT_MS).then(() => { late = true; return null; })]);
                if (bmp) return bmp;
            }
            const v = state.video;
        ...
        async function tick() {
            ...
            try {
                ...
                await onFrame(frame, state.cropped ? frame.width : rawW, state.cropped ? frame.height : rawH, freshness());
            } catch (e) {
        ...
        async function start() {
            ...
            Object.assign(state, {
                ...
                busy: false, rectKey: null, rect: null, cropped: null, sig: null, changedAt: 0
            });
            state.stopTicker = createTicker(intervalMs, tick);
            return true;
        }

        /* 지금 프레임 한 장을 바로 받는다 — { frame(캔버스, 게임 화면만), W, H, stale, age } 또는 null.
        ...
        async function grab() {
            ...
            try {
                ...
                return Object.assign({ frame, W: frame.width, H: frame.height }, freshness());
            } finally {
                if (raw && typeof ImageBitmap !== 'undefined' && raw instanceof ImageBitmap) raw.close();
                state.busy = false;
            }
        ...
        return { start, stop, isRunning, areaNote, grab, setFrameRate, isStale, frameAge };
    }
